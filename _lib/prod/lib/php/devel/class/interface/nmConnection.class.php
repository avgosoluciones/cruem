...
class nmConnection
{
    ...
    function TestConn($arr_conn)
    {
        ...
        $postgres_sslmode     = isset($arr_conn['postgres_sslmode']) ? $arr_conn['postgres_sslmode'] : "";
        $postgres_sslrootcert = isset($arr_conn['postgres_sslrootcert']) ? $arr_conn['postgres_sslrootcert'] : "";
        $postgres_sslkey      = isset($arr_conn['postgres_sslkey']) ? $arr_conn['postgres_sslkey'] : "";
        $postgres_sslcert     = isset($arr_conn['postgres_sslcert']) ? $arr_conn['postgres_sslcert'] : "";

        $oracle_type              = isset($arr_conn['oracle_type']) ? $arr_conn['oracle_type'] : "";

        $mssql_encrypt                = isset($arr_conn['mssql_encrypt']) ? $arr_conn['mssql_encrypt'] : "";
        $mssql_trustservercertificate = isset($arr_conn['mssql_trustservercertificate']) ? $arr_conn['mssql_trustservercertificate'] : "";
        $mssql_truststore             = isset($arr_conn['mssql_truststore']) ? $arr_conn['mssql_truststore'] : "";
        $mssql_truststorepassword     = isset($arr_conn['mssql_truststorepassword']) ? $arr_conn['mssql_truststorepassword'] : "";
        $mssql_hostnameincertificate  = isset($arr_conn['mssql_hostnameincertificate']) ? $arr_conn['mssql_hostnameincertificate'] : "";
        ...
        {
            $$_input = isset($arr_conn[ $_input ]) ? $arr_conn[ $_input ] : "";
        }
        ...
        else
        {
            ...
            elseif ($str_dbms == "db2_odbc")
            {
                ...
                foreach($arr_db2ssl as $_input)
                {
                    if(isset($arr_conn[ $_input ]) && !empty($arr_conn[ $_input ]))
                    {
                        $str_host .= ";". $_input ."=". $arr_conn[ $_input ];
                    }
                }
            }
            elseif ('postgres' == $str_dbms || 'postgres7' == $str_dbms || 'postgres8' == $str_dbms || 'postgres64' == $str_dbms)
            {
                ...
                if($use_ssl == 'Y') {
                    if (!empty($postgres_sslmode)) {
                        $str_host .= " sslmode=" . $postgres_sslmode;
                    }
                    if (!empty($postgres_sslrootcert)) {
                        $str_host .= " sslrootcert='" . str_replace("\\", "/", $postgres_sslrootcert) . "'";
                    }
                    if (!empty($postgres_sslkey)) {
                        $str_host .= " sslkey='" . str_replace("\\", "/", $postgres_sslkey) . "'";
                    }
                    if (!empty($postgres_sslcert)) {
                        $str_host .= " sslcert='" . str_replace("\\", "/", $postgres_sslcert) . "'";
                    }
                }
            ...
            elseif ('pdo_mysql' == $str_dbms || 'pdo_mariadb' == $str_dbms)
            {
            ...
            elseif ('pdo_sqlsrv' == $str_dbms)
            {
                ...
                if($mssql_encrypt == 'Y') {
                    ...
                    if(!empty($mssql_trustservercertificate) && $mssql_trustservercertificate == 'Y')
                    {
                        $str_host .= ";TrustServerCertificate=true";
                    }
                    else{
                        $str_host .= ";TrustServerCertificate=false";
                    }if(!empty($mssql_hostnameincertificate))
                    {
                        $str_host .= ";hostNameInCertificate=" . $mssql_hostnameincertificate;
                    }if(!empty($mssql_truststore))
                    {
                        $str_host .= ";truStstore=" . $mssql_truststore;
                    }
                    if(!empty($mssql_truststorepassword))
                    {
                        $str_host .= ";trustStorePassword=" . $mssql_truststorepassword;
                    }
                }
                else{
                    $str_host .= ";Encrypt=no";
            ...
            elseif ('mssql' == $str_dbms || 'mssqlnative' == $str_dbms)
            {
                ...
                if(!empty($mssql_encrypt) && $mssql_encrypt == 'Y')
                {
                    ...
                    if(!empty($mssql_hostnameincertificate))
                    {
                        $arrExtraArgs['hostNameInCertificate'] = $mssql_hostnameincertificate;
                    }
                    if(!empty($mssql_truststore))
                    {
                        $arrExtraArgs['truStstore'] = $mssql_truststore;
                    }
                    if(!empty($mssql_truststorepassword))
                    {
                        $arrExtraArgs['trustStorePassword'] = $mssql_truststorepassword;
                    }
                }
                else{
        ...
        $arr_group = [
            'azure' =>
                [
                    ...
                    'azure_mariadb' => $arr_sgdb['azure_mariadb']['azure_mariadb'],
            ...
            'googlecloud' =>
                [
                    ...
                    'googlecloud_mariadb' => $arr_sgdb['googlecloud_mariadb']['googlecloud_mariadb'],
            ...
            'amazonrds' =>
                [
                    ...
                    'amazonrds_mariadb' => $arr_sgdb['amazonrds_mariadb']['amazonrds_mariadb'],
    ...
    function GetSGBDVersions()
    {
        ...
        unset($arr_return['db2']['pdo_ibm']);
        unset($arr_return['db2']['pdo_db2_odbc']);
        unset($arr_return['db2']['odbc_db2']);
        unset($arr_return['db2']['odbc_db2v6']);
    ...
    function GetSGBDS()
    {
        ...
        $arr_ret = [
            ...
            'azure_mariadb' => [
                'azure_mariadb' => 'MariaDB'
            ],
            'azure_postgres' => [
                'azure_postgres' => 'PostgreSQL'
            ],
            'azure_mssql' => [
            ...
            'googlecloud_mariadb' => [
                'googlecloud_mariadb' => 'MariaDB'
            ],
            'googlecloud_postgres' => [
                'googlecloud_postgres' => 'PostgreSQL'
            ],
            'googlecloud_mssql' => [
            ...
            'amazonrds_mariadb' => [
                'amazonrds_mariadb' => 'MariaDB'
            ],
            'amazonrds_postgres' => [
                'amazonrds_postgres' => 'PostgreSQL'
            ],
            'amazonrds_mssql' => [
