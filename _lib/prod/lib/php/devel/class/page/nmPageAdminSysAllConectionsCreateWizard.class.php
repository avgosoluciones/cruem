...
class nmPageAdminSysAllConectionsCreateWizard extends nmPage
{
    ...
    function SaveConections($arr_fields, $str_prj="")
    {
        ...
        $arr_profile = array(
            ...
            'postgres_sslmode'       => nm_crypt_encode_utf8($arr_fields['postgres_sslmode']),
            'postgres_sslrootcert'       => nm_crypt_encode_utf8($arr_fields['postgres_sslrootcert']),
            'postgres_sslkey'       => nm_crypt_encode_utf8($arr_fields['postgres_sslkey']),
            'postgres_sslcert'       => nm_crypt_encode_utf8($arr_fields['postgres_sslcert']),
            'mssql_encrypt'       => nm_crypt_encode_utf8($arr_fields['mssql_encrypt']),
            'mssql_trustservercertificate'       => nm_crypt_encode_utf8($arr_fields['mssql_trustservercertificate']),
            'mssql_truststore'       => nm_crypt_encode_utf8($arr_fields['mssql_truststore']),
            'mssql_truststorepassword'       => nm_crypt_encode_utf8($arr_fields['mssql_truststorepassword']),
            'mssql_hostnameincertificate'       => nm_crypt_encode_utf8($arr_fields['mssql_hostnameincertificate']),
        );
        ...
        {
            $arr_profile[$_input] = nm_crypt_encode_utf8((isset($arr_fields[ $_input ])?$arr_fields[ $_input ]:""));
        }
    ...
    function iniSession()
    {
        ...
        $_SESSION['nm_session']['connection']['wizard']['postgres_sslmode']      = '';
        $_SESSION['nm_session']['connection']['wizard']['postgres_sslrootcert']      = '';
        $_SESSION['nm_session']['connection']['wizard']['postgres_sslkey']      = '';
        $_SESSION['nm_session']['connection']['wizard']['postgres_sslcert']      = '';

        $_SESSION['nm_session']['connection']['wizard']['oracle_type']      = '';

        $_SESSION['nm_session']['connection']['wizard']['mssql_encrypt']      = '';
        $_SESSION['nm_session']['connection']['wizard']['mssql_trustservercertificate']      = '';
        $_SESSION['nm_session']['connection']['wizard']['mssql_truststore']      = '';
        $_SESSION['nm_session']['connection']['wizard']['mssql_truststorepassword']      = '';
        $_SESSION['nm_session']['connection']['wizard']['mssql_hostnameincertificate']      = '';

        $_SESSION['nm_session']['connection']['wizard']['addgroup'] = 'S';
        $arr_ssh = array('use_ssh', 'ssh_server', 'ssh_user', 'ssh_port', 'ssh_privatecert', 'ssh_localportforwarding', 'ssh_localserver', 'ssh_localport');
        foreach($arr_ssh as $_input)
        {
            $_SESSION['nm_session']['connection']['wizard'][ $_input ] = '';
        }
        $arr_db2ssl = array('security', 'sslservercertificate', 'sslclientkeystoredb', 'sslclientkeystash', 'authentication', 'sslclientlabel', );
        foreach($arr_db2ssl as $_input)
        {
    ...
    {
        if($this->obj_conn->nm_db_sc_type($_SESSION['nm_session']['connection']['wizard']['dbms'])=="mysql" || $this->obj_conn->nm_db_sc_type($_SESSION['nm_session']['connection']['wizard']['dbms'])=="mariadb")
        {
    ...
    function DisplayContent()
    {
        ...
        $nm_template->Display('body_confirm');
        ...
        if ($this->FormSent('create'))
        {
            ...
            switch($this->str_page)
            {
                ...
                case 'dados_usu':
                    ...
                    $arr_fields['postgres_sslmode'] = $_SESSION['nm_session']['connection']['wizard']['postgres_sslmode'];
                    $arr_fields['postgres_sslrootcert'] = $_SESSION['nm_session']['connection']['wizard']['postgres_sslrootcert'];
                    $arr_fields['postgres_sslkey'] = $_SESSION['nm_session']['connection']['wizard']['postgres_sslkey'];
                    $arr_fields['postgres_sslcert'] = $_SESSION['nm_session']['connection']['wizard']['postgres_sslcert'];

                    $arr_fields['oracle_type'] = $_SESSION['nm_session']['connection']['wizard']['oracle_type'];

                    $arr_fields['mssql_encrypt'] = $_SESSION['nm_session']['connection']['wizard']['mssql_encrypt'];
                    $arr_fields['mssql_trustservercertificate'] = $_SESSION['nm_session']['connection']['wizard']['mssql_trustservercertificate'];
                    $arr_fields['mssql_truststore'] = $_SESSION['nm_session']['connection']['wizard']['mssql_truststore'];
                    $arr_fields['mssql_truststorepassword'] = $_SESSION['nm_session']['connection']['wizard']['mssql_truststorepassword'];
                    $arr_fields['mssql_hostnameincertificate'] = $_SESSION['nm_session']['connection']['wizard']['mssql_hostnameincertificate'];

                    $arr_ssh = array('use_ssh', 'ssh_server', 'ssh_user', 'ssh_port', 'ssh_privatecert', 'ssh_localportforwarding', 'ssh_localserver', 'ssh_localport');
                    foreach($arr_ssh as $_input)
                    {
                        $arr_fields[ $_input ] = (isset($_SESSION['nm_session']['connection']['wizard'][ $_input ])?$_SESSION['nm_session']['connection']['wizard'][ $_input ]:"");
                    }
                    $arr_db2ssl = array('security', 'sslservercertificate', 'sslclientkeystoredb', 'sslclientkeystash', 'authentication', 'sslclientlabel', );
                    foreach($arr_db2ssl as $_input)
                    {
    ...
    function PageJavascript()
    {
        ...
        $str_js  = " $(document).on('ready', function() { $('select').dropdown();  }); \n";
        ...
        $str_js .= "      return; \n";
        $str_js .= "   }\n";
        $str_js .= "   if (str_step == 'testar_conf' ) \n";
        $str_js .= "   {\n";
        $str_js .= "      return; \n";
        $str_js .= "   }\n";
        ...
        $str_js .= "            	if (str_start != null && str_start == 'S' && (nm_db_sc_type($('#dbms').val()) == 'mysql' || nm_db_sc_type($('#dbms').val()) == 'mariadb') && $('#server').val() != '' && $('#user').val() != '' && ($('#pass').val() != '' || ($('#pass').val() == '' && first_time == 'S'))) \n";
        $str_js .= "            	{\n";
        ...
        $str_js .= "            	    }\n";
        $str_js .= "            	}\n";
        $str_js .= "                $('select').dropdown();\n";
        $str_js .= "            }\n";
        $str_js .= "        });\n";
        $str_js .= "}\n";
        $this->AddJavascript($str_js);

        $str_js  = "function nm_test_conn(push_parent)\n";
        $str_js .= "{\n";
        $str_js .= "	if (typeof(push_parent) == 'undefined') {push_parent = false;} \n";
        ...
        $str_js .= "			}\n";
        $str_js .= "			if (document.form_create.elements[nI].name == 'use_ssl' && document.getElementById('use_ssl_y').checked != true)\n";
        $str_js .= "			{\n";
        $str_js .= "			    continue; \n";
        $str_js .= "			}\n";
        $str_js .= "			if (document.form_create.elements[nI].name == 'mssql_encrypt' && document.getElementById('mssql_encrypt').checked != true)\n";
        $str_js .= "			{\n";
        $str_js .= "			    continue; \n";
        $str_js .= "			}\n";
        $str_js .= "			if (document.form_create.elements[nI].name == 'mssql_trustservercertificate' && document.getElementById('mssql_trustservercertificate').checked != true)\n";
        $str_js .= "			{\n";
        $str_js .= "			    continue; \n";
        $str_js .= "			}\n";
        ...
        $str_js .= "{  \n";
        $str_js .= "    event.preventDefault();  \n";
        $str_js .= "	if ((nm_db_sc_type(document.form_create.dbms.value) == 'mysql' || nm_db_sc_type(document.form_create.dbms.value) == 'mariadb') && document.form_create.server && document.form_create.server.value == '') \n";
        $str_js .= "	{  \n";
        $str_js .= "		alert(\"". conv_utf8_all(html_entity_decode($nm_lang['msg_err_server_empty'])) ."\");  \n";
        $str_js .= "		document.form_create.server.focus();  \n";
        $str_js .= "		return;  \n";
        $str_js .= "	}  \n";
        $str_js .= "	if ((nm_db_sc_type(document.form_create.dbms.value) == 'mysql' || nm_db_sc_type(document.form_create.dbms.value) == 'mariadb') && document.form_create.user && document.form_create.user.value == '') \n";
        $str_js .= "	{  \n";
        $str_js .= "		nm_show_msg_modal(\"". conv_utf8_all(html_entity_decode($nm_lang['msg_err_user_empty'])) ."\");  \n";
...
//		$str_js .= "	document.getElementById('tr_title_extra').style.display = '';	\n";
        ...
        $str_js .= "	nm_test_conn(true);\n";
       // $str_js .= "    nm_show_msg_modal(); \n";
        $str_js .= "}  \n";
        $this->AddJavascript($str_js);

        $str_js  = "function nm_show_msg_modal(str_msg='') \n";
        $str_js .= "{ \n";
        $str_js .= "    if(str_msg != ''){ \n";
        $str_js .= "    $('#id_block_message_content').html(str_msg); \n";
        $str_js .= "    } \n";
        $str_js .= "    $('#id_block_message').modal({ \n";
        $str_js .= "        blurring: true, \n";
        $str_js .= "        onHidden: function() \n";
        $str_js .= "        { \n";
        $str_js .= "            setStep('testar');\n";
        $str_js .= "        } \n";
        $str_js .= "    }).modal('show'); \n";
        $str_js .= "} \n";
        $this->AddJavascript($str_js);

        $str_js  = "function nm_show_confirm_modal() \n";
        $str_js .= "{ \n";
        $str_js .= "    $('#confirm_message').modal({ \n";
        $str_js .= "        blurring: true, \n";
        $str_js .= "        onApprove: function() \n";
        $str_js .= "        { \n";
        $str_js .= "            nm_show_msg_modal();\n";
        $str_js .= "        } \n";
        $str_js .= "    }).modal('show'); \n";
...
//		$str_js .= "	document.getElementById('bt_nav_1').style.display = '';	\n";
        ...
        $str_js .= "  if((nm_db_sc_type($('#dbms').val()) == 'mysql' || nm_db_sc_type($('#dbms').val()) == 'mariadb') && $('#use_ssl_y:checked').length>0 && $('#use_ssl_y:checked').val()=='Y')\n";
        $str_js .= "  {\n";
        $str_js .= "      str_add_parameters +='&use_ssl='+$('#use_ssl_y:checked').val();\n";
        ...
        $str_js .= "  }\n";
        $str_js .= "  else if(nm_db_sc_type($('#dbms').val()) == 'postgres' && $('#use_ssl_y:checked').length>0 && $('#use_ssl_y:checked').val()=='Y')\n";
        $str_js .= "  {\n";
        $str_js .= "      str_add_parameters +='&use_ssl='+$('#use_ssl_y:checked').val();\n";
        $str_js .= "      str_add_parameters +='&postgres_sslmode='+$('#postgres_sslmode').val();\n";
        $str_js .= "      str_add_parameters +='&postgres_sslrootcert='+$('#postgres_sslrootcert').val();\n";
        $str_js .= "      str_add_parameters +='&postgres_sslkey='+$('#postgres_sslkey').val();\n";
        $str_js .= "      str_add_parameters +='&postgres_sslcert='+$('#postgres_sslcert').val();\n";
        $str_js .= "  }\n";
        $str_js .= "  else if(nm_db_sc_type($('#dbms').val()) == 'mssql' && $('#mssql_encrypt:checked').length>0 && $('#mssql_encrypt:checked').val()=='Y')\n";
        $str_js .= "  {\n";
        $str_js .= "      str_add_parameters +='&mssql_encrypt='+$('#mssql_encrypt:checked').val();\n";
        $str_js .= "      str_add_parameters +='&mssql_trustservercertificate='+$('#mssql_trustservercertificate:checked').val();\n";
        $str_js .= "      str_add_parameters +='&mssql_truststore='+$('#mssql_truststore').val();\n";
        $str_js .= "      str_add_parameters +='&mssql_truststorepassword='+$('#mssql_truststorepassword').val();\n";
        $str_js .= "      str_add_parameters +='&mssql_hostnameincertificate='+$('#mssql_hostnameincertificate').val();\n";
        $str_js .= "  }\n";
        $str_js .= "    arr_extras = ['ssh_server', 'ssh_user', 'ssh_port', 'ssh_privatecert', 'ssh_localserver', 'ssh_localport', 'ssh_localportforwarding', 'security', 'sslservercertificate', 'sslclientkeystoredb', 'sslclientkeystash', 'authentication', 'sslclientlabel'];\n";
        $str_js .= "    str_add_parameters +='&use_ssh='+$('input[name=use_ssh]:checked').val();\n";
        $str_js .= "	arr_extras.forEach((item, index) => {\n";
        $str_js .= "	if($('#' + item).length > 0)\n";
        $str_js .= "	    str_add_parameters +='&' + item + '='+$('#' + item).val();\n";
        $str_js .= "	});\n";
        $str_js .= "	$.ajax({\n";
        $str_js .= "            type: 'POST',\n";
        $str_js .= "            url: '" . $nm_config['url_iface'] . 'admin_sys_allconections_create_wizard.php' ."',\n";
        ...
        $str_js .= "            	else\n";
        $str_js .= "            	{\n";
        $str_js .= "                    $('#span_sel_database_name').html(\"<select class='nmInput' ><option>&nbsp;</option></select>\");  \n";
        $str_js .= "            		if (nome_db == null || nome_db == '')  \n";
        $str_js .= "            		{\n";
        ...
        */
        ...
        $str_js .= " {\n";
        $str_js .= "    $('#id_modal_del_confirm_content').html('<h4>".conv_utf8_all(html_entity_decode($msg_exc))."</h4>');\n";
        $str_js .= "    $('#id_modal_del_confirm').modal({\n";
        $str_js .= "        blurring: true,\n";
        $str_js .= "        onApprove: function(){\n";
...
EOT;
    ...
    function ValidateField($v_str_field)
    {
        ...
        switch ($v_str_field)
        {
            ...
            case 'postgres_sslrootcert':
            case 'postgres_sslkey':
            case 'postgres_sslcert':
            case 'oracle_type':
            case 'mssql_encrypt':
            case 'mssql_trustservercertificate':
            case 'mssql_truststore':
            case 'mssql_truststorepassword':
            case 'mssql_hostnameincertificate':
            case 'use_ssh':
            case 'ssh_server':
            case 'ssh_user':
            case 'ssh_port':
            case 'ssh_privatecert':
            case 'ssh_localportforwarding':
            case 'ssh_localserver':
            case 'ssh_localport':
            case 'security':
            case 'sslservercertificate':
            case 'sslclientkeystoredb':
            case 'sslclientkeystash':
            case 'authentication':
            case 'sslclientlabel':
            $_SESSION['nm_session']['connection']['wizard'][$v_str_field] = $mix_val;
                break;
    ...
    function PrepareEditConn()
    {
        ...
        if ($this->IsArg('flag_edit') && $this->GetArg('flag_edit') == 'S')
        {
            ...
            $_SESSION['nm_session']['connection']['wizard']['postgres_sslmode'] = nm_crypt_decode($arr_conn['postgres_sslmode']);
            $_SESSION['nm_session']['connection']['wizard']['postgres_sslrootcert'] = nm_crypt_decode($arr_conn['postgres_sslrootcert']);
            $_SESSION['nm_session']['connection']['wizard']['postgres_sslkey'] = nm_crypt_decode($arr_conn['postgres_sslkey']);
            $_SESSION['nm_session']['connection']['wizard']['postgres_sslcert'] = nm_crypt_decode($arr_conn['postgres_sslcert']);

            $_SESSION['nm_session']['connection']['wizard']['mssql_encrypt'] = nm_crypt_decode($arr_conn['mssql_encrypt']);
            $_SESSION['nm_session']['connection']['wizard']['mssql_trustservercertificate'] = nm_crypt_decode($arr_conn['mssql_trustservercertificate']);
            $_SESSION['nm_session']['connection']['wizard']['mssql_truststore'] = nm_crypt_decode($arr_conn['mssql_truststore']);
            $_SESSION['nm_session']['connection']['wizard']['mssql_truststorepassword'] = nm_crypt_decode($arr_conn['mssql_truststorepassword']);
            $_SESSION['nm_session']['connection']['wizard']['mssql_hostnameincertificate'] = nm_crypt_decode($arr_conn['mssql_hostnameincertificate']);
            ...
            foreach($arr_ssh as $_input)
            {
                $_SESSION['nm_session']['connection']['wizard'][ $_input ] = nm_crypt_decode((isset($arr_conn[ $_input ])?$arr_conn[ $_input ]:""));
            }
            $arr_db2ssl = array('security', 'sslservercertificate', 'sslclientkeystoredb', 'sslclientkeystash', 'authentication', 'sslclientlabel', );
            foreach($arr_db2ssl as $_input)
            {
    ...
    function GetListDatabaseNameMySql($host, $usr, $pwd, $driver, $v_arr_db)
    {
        ...
        if(isset($v_arr_db['use_ssh']) && $v_arr_db['use_ssh'] =='Y')
        {
            $localPort = (isset($v_arr_db['ssh_localportforwarding']) && !empty($v_arr_db['ssh_localportforwarding']))?$v_arr_db['ssh_localportforwarding']:"";
            $server = (isset($v_arr_db['ssh_localserver']) && !empty($v_arr_db['ssh_localserver']))?$v_arr_db['ssh_localserver']:"";
            $port = (isset($v_arr_db['ssh_localport']) && !empty($v_arr_db['ssh_localport']))?$v_arr_db['ssh_localport']:"";
            $sshUser = (isset($v_arr_db['ssh_user']) && !empty($v_arr_db['ssh_user']))?$v_arr_db['ssh_user']:"";
            $sshHost = (isset($v_arr_db['ssh_server']) && !empty($v_arr_db['ssh_server']))?$v_arr_db['ssh_server']:"";
            $sshPort = (isset($v_arr_db['ssh_port']) && !empty($v_arr_db['ssh_port']))?$v_arr_db['ssh_port']:"";
            $cert = (isset($v_arr_db['ssh_privatecert']) && !empty($v_arr_db['ssh_privatecert']))?$v_arr_db['ssh_privatecert']:"";
        ...
        try {
            if($driver == 'pdo_mysql' || $driver == 'pdo_mariadb')
            {
    ...
    function GetDbCharset($host, $db, $usr, $pwd, $driver)
    {
        ...
        elseif($driver == 'pdo_mysql' || $driver == 'pdo_mariadb')
        {
            list($host, $port) = explode(':', $host);
    ...
    function Ajax()
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] == 'S')
        {
            if (isset($_POST['list_db']) && $_POST['list_db'] == 'S')
            {
                ...
                if($_POST['dbms'] == 'mysql' || $_POST['dbms'] == 'mariadb')
                {
                ...
                elseif($_POST['dbms'] == 'mssql')
                {
                    if(isset($_POST['mssql_encrypt']))
                    {
                        $v_arr_db['mssql_encrypt']          = $this->unProtectAjaxChar($_POST['mssql_encrypt']);
                        $v_arr_db['mssql_trustservercertificate']    = $this->unProtectAjaxChar($_POST['mssql_trustservercertificate']);
                        $v_arr_db['mssql_truststore']   = $this->unProtectAjaxChar($_POST['mssql_truststore']);
                        $v_arr_db['mssql_truststorepassword'] = $this->unProtectAjaxChar($_POST['mssql_truststorepassword']);
                        $v_arr_db['mssql_hostnameincertificate']     = $this->unProtectAjaxChar($_POST['mssql_hostnameincertificate']);
                    }
                }
    ...
    function GetConnection($str_conn)
    {
        ...
        $arr_conn['postgres_sslmode'] = (isset($arr_ini_conn['postgres_sslmode'])?$arr_ini_conn['postgres_sslmode']:"");
        $arr_conn['postgres_sslrootcert'] = (isset($arr_ini_conn['postgres_sslrootcert'])?$arr_ini_conn['postgres_sslrootcert']:"");
        $arr_conn['postgres_sslkey'] = (isset($arr_ini_conn['postgres_sslkey'])?$arr_ini_conn['postgres_sslkey']:"");
        $arr_conn['postgres_sslcert'] = (isset($arr_ini_conn['postgres_sslcert'])?$arr_ini_conn['postgres_sslcert']:"");

        $arr_conn['oracle_type'] = $arr_ini_conn['oracle_type'];

        $arr_conn['mssql_encrypt'] = $arr_ini_conn['mssql_encrypt'];
        $arr_conn['mssql_trustservercertificate'] = $arr_ini_conn['mssql_trustservercertificate'];
        $arr_conn['mssql_truststore'] = $arr_ini_conn['mssql_truststore'];
        $arr_conn['mssql_truststorepassword'] = $arr_ini_conn['mssql_truststorepassword'];
        $arr_conn['mssql_hostnameincertificate'] = $arr_ini_conn['mssql_hostnameincertificate'];

        $arr_ssh = array('use_ssh', 'ssh_server', 'ssh_user', 'ssh_port', 'ssh_privatecert', 'ssh_localportforwarding', 'ssh_localserver', 'ssh_localport');
        foreach($arr_ssh as $_input)
        {
            $arr_conn[$_input] = (isset($arr_ini_conn[ $_input ])?$arr_ini_conn[ $_input ]:"");
        }
        $arr_db2ssl = array('security', 'sslservercertificate', 'sslclientkeystoredb', 'sslclientkeystash', 'authentication', 'sslclientlabel', );
        foreach($arr_db2ssl as $_input)
        {
