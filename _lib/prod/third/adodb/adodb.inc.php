...
*/
	...
	*/
	 ...
	 */
	function Insert_ID($table='', $column='', $sequence='')
	{
		if ($this->_logsql && $this->lastInsID) return $this->lastInsID;
		if (!empty($sequence) || $this->hasInsertID)
        {
            return $this->_insertid($table, $column, $sequence);
        }
		if ($this->debug) {
	...
	function lastInsertedId($table="", $id="", $sequence="")
	{
	   if (!empty($sequence) || $this->hasInsertID){
		   return $this->Insert_ID($table, $id, $sequence);
	   } else {
		   return $this->GetOne("SELECT MAX($id) FROM $table");
	   }
	}
...
*/
	...
	*/
    ...
    function startSSH($arr_ssh)
    {
        ...
        {
            $time_start = $this->microtime_float();
            $this->sc_start_debug("Connect SSH", "", $time_start);
        }
        ...
        {
            if(!is_file($arr_ssh['ssh_privatecert']))
            {
                $ret = false;
                if ($fn = $this->raiseErrorFn)
                    $fn($this->databaseType, 'SSH', $this->ErrorNo(), 'SSH PRivate cert not a file: ' . $arr_ssh['ssh_privatecert'], $this->host, $this->database, $this);
            }
            else
            {
        ...
        if ($this->debug) ADOConnection::outp( $str_command . ' ' . $str_error);
