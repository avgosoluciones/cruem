...
$old_error_handler = set_error_handler("app3_add_2fa_mob_scriptcase_error_handler");

function app3_add_2fa_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['app3_add_2fa_mob']['contr_erro']) && $_SESSION['scriptcase']['app3_add_2fa_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_app3_add_2fa_mob->contr_app3_add_2fa_mob->Apl_com_erro = true;
            if ($inicial_app3_add_2fa_mob->contr_app3_add_2fa_mob->NM_ajax_flag)
            {
                $inicial_app3_add_2fa_mob->contr_app3_add_2fa_mob->NM_ajax_info['errList']['geral_app3_add_2fa_mob'][] = $msg;
                return;
            }
            app3_add_2fa_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function app3_add_2fa_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['app3_add_2fa_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['app3_add_2fa_mob'])  ? $_SESSION['scriptcase']['error_icon']['app3_add_2fa_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['app3_add_2fa_mob']) && '' != $_SESSION['scriptcase']['error_close']['app3_add_2fa_mob']) ? $_SESSION['scriptcase']['error_close']['app3_add_2fa_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['app3_add_2fa_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['app3_add_2fa_mob']) ? $_SESSION['scriptcase']['error_span_title']['app3_add_2fa_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['app3_add_2fa_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['app3_add_2fa_mob']) ? $_SESSION['scriptcase']['error_icon_title']['app3_add_2fa_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['app3_add_2fa']['error_buffer'] .= ob_get_flush();
}
