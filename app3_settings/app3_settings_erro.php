...
$old_error_handler = set_error_handler("app3_settings_scriptcase_error_handler");

function app3_settings_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['app3_settings']['contr_erro']) && $_SESSION['scriptcase']['app3_settings']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_app3_settings->contr_app3_settings->Apl_com_erro = true;
            if ($inicial_app3_settings->contr_app3_settings->NM_ajax_flag)
            {
                $inicial_app3_settings->contr_app3_settings->NM_ajax_info['errList']['geral_app3_settings'][] = $msg;
                return;
            }
            app3_settings_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function app3_settings_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['app3_settings'])  && '' != $_SESSION['scriptcase']['error_icon']['app3_settings'])  ? $_SESSION['scriptcase']['error_icon']['app3_settings']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['app3_settings']) && '' != $_SESSION['scriptcase']['error_close']['app3_settings']) ? $_SESSION['scriptcase']['error_close']['app3_settings'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['app3_settings']) && '' != $_SESSION['scriptcase']['error_span_title']['app3_settings']) ? $_SESSION['scriptcase']['error_span_title']['app3_settings'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['app3_settings']) && '' != $_SESSION['scriptcase']['error_icon_title']['app3_settings']) ? $_SESSION['scriptcase']['error_icon_title']['app3_settings'] : '';
?>
