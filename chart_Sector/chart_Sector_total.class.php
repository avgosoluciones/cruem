...
class chart_Sector_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_Sector']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_Sector']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false, $ind_compara="", $tipo="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('sector_sector' => "sector.Sector");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
          $cmps_quebra_atual[] = $cmp;
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['SC_force_fiels_gb'];
      }
      $Save_Format = '';
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['SC_clear_total']);
      $ind_cmps  = 1;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export, $Save_Format)
   {
      ...
      $cmp_sql_tp_num = array('procedimiento1_id_procedimiento' => 'N','procedimiento1_id_barrio' => 'N','procedimiento1_edad' => 'N','procedimiento1_id_seguridad_social' => 'N','procedimiento1_id_eps' => 'N','procedimiento1_id_movil' => 'N','procedimiento1_id_ips' => 'N','procedimiento1_id_medico' => 'N','procedimiento1_id_tipo_evento' => 'N','barrio_id_barrio' => 'N','barrio_id_sector' => 'N','sector_id_sector' => 'N');
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['sector_sector']['reg'] = "S";
      $format_dimensions['sector_sector']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_Sector']['contr_total_geral'] = "OK";
   } 

   //-----  sector_sector
   function quebra_sector_sector_sc_free_group_by($sector_sector, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_sector_sector = array() ;  
      $tot_sector_sector[0] = $sector_sector ; 
   }
