...
$old_error_handler = set_error_handler("editar_procedimiento_scriptcase_error_handler");

function editar_procedimiento_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['editar_procedimiento']['contr_erro']) && $_SESSION['scriptcase']['editar_procedimiento']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_editar_procedimiento->contr_editar_procedimiento->Apl_com_erro = true;
            if ($inicial_editar_procedimiento->contr_editar_procedimiento->NM_ajax_flag)
            {
                $inicial_editar_procedimiento->contr_editar_procedimiento->NM_ajax_info['errList']['geral_editar_procedimiento'][] = $msg;
                return;
            }
            editar_procedimiento_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function editar_procedimiento_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['editar_procedimiento'])  && '' != $_SESSION['scriptcase']['error_icon']['editar_procedimiento'])  ? $_SESSION['scriptcase']['error_icon']['editar_procedimiento']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['editar_procedimiento']) && '' != $_SESSION['scriptcase']['error_close']['editar_procedimiento']) ? $_SESSION['scriptcase']['error_close']['editar_procedimiento'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['editar_procedimiento']) && '' != $_SESSION['scriptcase']['error_span_title']['editar_procedimiento']) ? $_SESSION['scriptcase']['error_span_title']['editar_procedimiento'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['editar_procedimiento']) && '' != $_SESSION['scriptcase']['error_icon_title']['editar_procedimiento']) ? $_SESSION['scriptcase']['error_icon_title']['editar_procedimiento'] : '';
?>
