...
$old_error_handler = set_error_handler("editar_procedimiento_mob_scriptcase_error_handler");

function editar_procedimiento_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['editar_procedimiento_mob']['contr_erro']) && $_SESSION['scriptcase']['editar_procedimiento_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_editar_procedimiento_mob->contr_editar_procedimiento_mob->Apl_com_erro = true;
            if ($inicial_editar_procedimiento_mob->contr_editar_procedimiento_mob->NM_ajax_flag)
            {
                $inicial_editar_procedimiento_mob->contr_editar_procedimiento_mob->NM_ajax_info['errList']['geral_editar_procedimiento_mob'][] = $msg;
                return;
            }
            editar_procedimiento_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function editar_procedimiento_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['editar_procedimiento_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['editar_procedimiento_mob'])  ? $_SESSION['scriptcase']['error_icon']['editar_procedimiento_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['editar_procedimiento_mob']) && '' != $_SESSION['scriptcase']['error_close']['editar_procedimiento_mob']) ? $_SESSION['scriptcase']['error_close']['editar_procedimiento_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['editar_procedimiento_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['editar_procedimiento_mob']) ? $_SESSION['scriptcase']['error_span_title']['editar_procedimiento_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['editar_procedimiento_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['editar_procedimiento_mob']) ? $_SESSION['scriptcase']['error_icon_title']['editar_procedimiento_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['editar_procedimiento']['error_buffer'] .= ob_get_flush();
}
