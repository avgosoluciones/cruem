  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_barrio_mob" == sTestField)
        {
...
?>
  ...
  // ---------- Validate id_barrio
  function do_ajax_form_barrio_mob_validate_id_barrio()
  {
    var nomeCampo_id_barrio = "id_barrio";
    var var_id_barrio = scAjaxGetFieldHidden(nomeCampo_id_barrio);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_barrio_mob_validate_id_barrio(var_id_barrio, var_script_case_init, do_ajax_form_barrio_mob_validate_id_barrio_cb);
  } // do_ajax_form_barrio_mob_validate_id_barrio

  function do_ajax_form_barrio_mob_validate_id_barrio_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_barrio_mob_validate_id_barrio_cb

  // ---------- Validate barrio
  function do_ajax_form_barrio_mob_validate_barrio()
  {
    var nomeCampo_barrio = "barrio";
    var var_barrio = scAjaxGetFieldText(nomeCampo_barrio);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_barrio_mob_validate_barrio(var_barrio, var_script_case_init, do_ajax_form_barrio_mob_validate_barrio_cb);
  } // do_ajax_form_barrio_mob_validate_barrio

  function do_ajax_form_barrio_mob_validate_barrio_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "barrio";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_barrio_mob_validate_barrio_cb

  // ---------- Validate id_sector
  function do_ajax_form_barrio_mob_validate_id_sector()
  {
    var nomeCampo_id_sector = "id_sector";
    var var_id_sector = scAjaxGetFieldSelect(nomeCampo_id_sector);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_barrio_mob_validate_id_sector(var_id_sector, var_script_case_init, do_ajax_form_barrio_mob_validate_id_sector_cb);
  } // do_ajax_form_barrio_mob_validate_id_sector

  function do_ajax_form_barrio_mob_validate_id_sector_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "id_sector";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_barrio_mob_validate_id_sector_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_form_barrio_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_form_barrio_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_id_barrio = scAjaxGetFieldHidden("id_barrio");
    var var_barrio = scAjaxGetFieldText("barrio");
    var var_id_sector = scAjaxGetFieldSelect("id_sector");
    ...
    scAjaxProcOn();
    x_ajax_form_barrio_mob_submit_form(var_id_barrio, var_barrio, var_id_sector, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_barrio_mob_submit_form_cb);
  } // do_ajax_form_barrio_mob_submit_form

  function do_ajax_form_barrio_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scAjaxHideErrorDisplay("id_sector");
      scLigEditLookupCall();
<?php
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_form_barrio_mob_submit_form_cb_after_alert);
  } // do_ajax_form_barrio_mob_submit_form_cb
  function do_ajax_form_barrio_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_barrio_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_barrio_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_barrio_mob_navigate_form, function() {});
    } else {
      perform_ajax_form_barrio_mob_navigate_form();
    }
  }

  function perform_ajax_form_barrio_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    scAjaxHideErrorDisplay("barrio");
    scAjaxHideErrorDisplay("id_sector");
    var var_id_barrio = document.F2.id_barrio.value;
    ...
    scAjaxProcOn();
    x_ajax_form_barrio_mob_navigate_form(var_id_barrio, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search,  var_nmgp_cond_fast_search,  var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_barrio_mob_navigate_form_cb);
  } // do_ajax_form_barrio_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_barrio_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_barrio_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    scAjaxSetVariables();
    scAjaxSetSummaryLine();
    document.F2.id_barrio.value = scAjaxGetKeyValue("id_barrio");
    scAjaxSetSummary();
    scAjaxShowDebug();
    scAjaxSetLabel(true);
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_barrio_mob_navigate_form_cb_after_alert);
  } // do_ajax_form_barrio_mob_navigate_form_cb
  function do_ajax_form_barrio_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
{
?>
    do_ajax_form_barrio_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_form_barrio_mob_navigate_form_cb_after_alert

  function sc_hide_form_barrio_mob_form()
  {
  ...
  ajax_field_list[0] = "id_barrio";
  ajax_field_list[1] = "barrio";
  ajax_field_list[2] = "id_sector";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "id_barrio": {"label": "Id Barrio", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 0},
    "barrio": {"label": "Barrio", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 0},
    "id_sector": {"label": "Id Sector", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 0}
  };
  ...
  ajax_field_mult["id_sector"][1] = "id_sector";
  ...
  var ajax_field_id = {
    ...
    "id_sector": new Array("hidden_field_label_id_sector", "hidden_field_data_id_sector")
  };
