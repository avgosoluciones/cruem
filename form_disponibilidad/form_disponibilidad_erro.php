...
$old_error_handler = set_error_handler("form_disponibilidad_scriptcase_error_handler");

function form_disponibilidad_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_disponibilidad']['contr_erro']) && $_SESSION['scriptcase']['form_disponibilidad']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_disponibilidad->contr_form_disponibilidad->Apl_com_erro = true;
            if ($inicial_form_disponibilidad->contr_form_disponibilidad->NM_ajax_flag)
            {
                $inicial_form_disponibilidad->contr_form_disponibilidad->NM_ajax_info['errList']['geral_form_disponibilidad'][] = $msg;
                return;
            }
            form_disponibilidad_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_disponibilidad_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_disponibilidad'])  && '' != $_SESSION['scriptcase']['error_icon']['form_disponibilidad'])  ? $_SESSION['scriptcase']['error_icon']['form_disponibilidad']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_disponibilidad']) && '' != $_SESSION['scriptcase']['error_close']['form_disponibilidad']) ? $_SESSION['scriptcase']['error_close']['form_disponibilidad'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_disponibilidad']) && '' != $_SESSION['scriptcase']['error_span_title']['form_disponibilidad']) ? $_SESSION['scriptcase']['error_span_title']['form_disponibilidad'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_disponibilidad']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_disponibilidad']) ? $_SESSION['scriptcase']['error_icon_title']['form_disponibilidad'] : '';
?>
