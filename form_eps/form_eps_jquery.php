...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["eps_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["eps_" + iSeqRow] && scEventControl_data["eps_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["eps_" + iSeqRow] && scEventControl_data["eps_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_eps_' + iSeqRow).bind('blur', function() { sc_form_eps_id_eps__onblur('#id_sc_field_id_eps_' + iSeqRow, iSeqRow) })
                                     .bind('change', function() { sc_form_eps_id_eps__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_eps_id_eps__onfocus(this, iSeqRow) });
  $('#id_sc_field_eps_' + iSeqRow).bind('blur', function() { sc_form_eps_eps__onblur('#id_sc_field_eps_' + iSeqRow, iSeqRow) })
                                  .bind('change', function() { sc_form_eps_eps__onchange(this, iSeqRow) })
                                  .bind('focus', function() { sc_form_eps_eps__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_eps_id_eps__onblur(oThis, iSeqRow) {
  do_ajax_form_eps_validate_id_eps_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_eps_id_eps__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_eps_id_eps__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_eps_eps__onblur(oThis, iSeqRow) {
  do_ajax_form_eps_validate_eps_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_eps_eps__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_eps_eps__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
        ...
        displayChange_field("eps_", "", status);
}

function displayChange_row(row, status) {
        displayChange_field_id_eps_(row, status);
        displayChange_field_eps_(row, status);
}
...
function displayChange_field_eps_(row, status) {
    var fieldId;
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_eps')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
