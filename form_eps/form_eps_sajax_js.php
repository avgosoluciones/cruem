...
?>
  ...
  // ---------- Validate id_eps_
  function do_ajax_form_eps_validate_id_eps_(iNumLinha)
  {
    var nomeCampo_id_eps_ = "id_eps_" + iNumLinha;
    var var_id_eps_ = scAjaxGetFieldHidden(nomeCampo_id_eps_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_eps_validate_id_eps_(var_id_eps_, iNumLinha, var_script_case_init, do_ajax_form_eps_validate_id_eps__cb);
  } // do_ajax_form_eps_validate_id_eps_

  function do_ajax_form_eps_validate_id_eps__cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_eps_validate_id_eps__cb

  // ---------- Validate eps_
  function do_ajax_form_eps_validate_eps_(iNumLinha)
  {
    var nomeCampo_eps_ = "eps_" + iNumLinha;
    var var_eps_ = scAjaxGetFieldText(nomeCampo_eps_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_eps_validate_eps_(var_eps_, iNumLinha, var_script_case_init, do_ajax_form_eps_validate_eps__cb);
  } // do_ajax_form_eps_validate_eps_

  function do_ajax_form_eps_validate_eps__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    ...
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "eps_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "eps_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_eps_validate_eps__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_eps_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_eps_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_eps_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_eps_add_new_line_cb);
  } // do_ajax_form_eps_add_new_line

  function do_ajax_form_eps_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_eps_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_eps_backup_line(iNumLinha)
  {
    var var_id_eps_ = scAjaxGetFieldHidden("id_eps_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_eps_backup_line(var_id_eps_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_eps_backup_line_cb);
  } // do_ajax_form_eps_backup_line

  function do_ajax_form_eps_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_eps_backup_line_cb

  function do_ajax_form_eps_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_eps_cancel_insert

  function do_ajax_form_eps_cancel_update(iSeqVert)
  {
    do_ajax_form_eps_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_eps_cancel_update

  function do_ajax_form_eps_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_eps_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_eps_table_refresh()
  {
    var var_id_eps_ = document.F2.id_eps_.value;
    ...
    scAjaxProcOn();
    x_ajax_form_eps_table_refresh(var_id_eps_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search, var_nmgp_cond_fast_search, var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_eps_table_refresh_cb);
  } //  do_ajax_form_eps_table_refresh

  function do_ajax_form_eps_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    document.F2.id_eps_.value = scAjaxGetKeyValue("id_eps_");
    ...
    scAjaxAlert(do_ajax_form_eps_table_refresh_cb_after_alert);
  } // do_ajax_form_eps_table_refresh_cb
  function do_ajax_form_eps_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_form_eps_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_eps_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_eps_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    ...
    scAjaxHideMessage();
    var var_id_eps_ = scAjaxGetFieldHidden("id_eps_" + iNumLinha);
    var var_eps_ = scAjaxGetFieldText("eps_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_eps_submit_form(var_id_eps_, var_eps_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_eps_submit_form_cb);
  } // do_ajax_form_eps_submit_form

  function do_ajax_form_eps_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("eps_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_eps_submit_form_cb_after_alert);
  } // do_ajax_form_eps_submit_form_cb
  function do_ajax_form_eps_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_eps_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_eps_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_eps_navigate_form, function() {});
    } else {
      perform_ajax_form_eps_navigate_form();
    }
  }

  function perform_ajax_form_eps_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    for (iNavForm = 1; iNavForm < <?php echo $this->sc_max_reg; ?> + 1; iNavForm++)
    {
      ...
      scAjaxHideErrorDisplay("eps_" + iNavForm);
    }
    var var_id_eps_ = document.F2.id_eps_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_eps_navigate_form(var_id_eps_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_eps_navigate_form_cb);
  } // do_ajax_form_eps_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_eps']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_eps_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    scAjaxSetVariables();
    scAjaxSetSummaryLine();
    document.F2.id_eps_.value = scAjaxGetKeyValue("id_eps_" + var_last_index);
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_eps_navigate_form_cb_after_alert);
  } // do_ajax_form_eps_navigate_form_cb
  function do_ajax_form_eps_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_form_eps_navigate_form_cb_after_alert

    function sc_form_eps_get_last_line_number()
    {
    ...
    } //sc_form_eps_get_last_line_number

  function sc_hide_form_eps_form()
  {
...
?>
  function ajax_create_tables(iNumLinha)
  {
    ...
    iTotCampos++;
    ajax_field_list[iTotCampos] = "eps_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["id_eps_" + iNumLinha] = {"label": "Id Eps<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["eps_" + iNumLinha] = {"label": "Eps<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["id_eps_"][iNumLinha] = "id_eps_" + iNumLinha;
    ajax_field_mult["eps_"][iNumLinha] = "eps_" + iNumLinha;
    ajax_field_id["id_eps_" + iNumLinha] = new Array("hidden_field_label_id_eps_", "hidden_field_data_id_eps_" + iNumLinha);
    ajax_field_id["eps_" + iNumLinha] = new Array("hidden_field_label_eps_", "hidden_field_data_eps_" + iNumLinha);
    ajax_error_count["id_eps_" + iNumLinha] = "off";
    ajax_error_count["eps_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["id_eps_" + iNumLinha] = "on";
    ajax_read_only["eps_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ajax_read_only["id_eps_" + iNumLinha] = "on";
    ajax_read_only["eps_" + iNumLinha] = "on";
<?php
}
?>
  }
  function ajax_destroy_tables(iNumLinha)
  {
    ...
    ajax_error_list["eps_" + iNumLinha] = null;
  }
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ...
      ajax_error_list["eps_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["eps_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["eps_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["eps_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["eps_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['id_eps_'])) ? $this->nmgp_cmp_readonly['id_eps_'] : 'on';
?>
    scAjaxFieldRead("id_eps_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['eps_'])) ? $this->nmgp_cmp_readonly['eps_'] : 'off';
?>
    scAjaxFieldRead("eps_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("id_eps_" + iSeq, "on");
    scAjaxFieldRead("eps_" + iSeq, "on");
    rerunHeaderDisplay = 1;
    scSetFixedHeaders(true);
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["id_eps_" + iRow] || "on" == ajax_error_count["eps_" + iRow]))
    {
      ...
      $("#hidden_field_data_eps_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["id_eps_" + iRow] && "off" == ajax_error_count["eps_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["id_eps_" + iRow] = "off";
        ajax_error_count["eps_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_eps_" + iRow).removeClass("scFormErrorLine");
    }
  }
