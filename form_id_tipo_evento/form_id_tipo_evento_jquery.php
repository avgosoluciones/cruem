...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["evento_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["evento_" + iSeqRow] && scEventControl_data["evento_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["evento_" + iSeqRow] && scEventControl_data["evento_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_tipo_evento_' + iSeqRow).bind('blur', function() { sc_form_id_tipo_evento_id_tipo_evento__onblur('#id_sc_field_id_tipo_evento_' + iSeqRow, iSeqRow) })
                                             .bind('change', function() { sc_form_id_tipo_evento_id_tipo_evento__onchange(this, iSeqRow) })
                                             .bind('focus', function() { sc_form_id_tipo_evento_id_tipo_evento__onfocus(this, iSeqRow) });
  $('#id_sc_field_evento_' + iSeqRow).bind('blur', function() { sc_form_id_tipo_evento_evento__onblur('#id_sc_field_evento_' + iSeqRow, iSeqRow) })
                                     .bind('change', function() { sc_form_id_tipo_evento_evento__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_id_tipo_evento_evento__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_id_tipo_evento_id_tipo_evento__onblur(oThis, iSeqRow) {
  do_ajax_form_id_tipo_evento_validate_id_tipo_evento_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_id_tipo_evento_id_tipo_evento__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_id_tipo_evento_id_tipo_evento__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_id_tipo_evento_evento__onblur(oThis, iSeqRow) {
  do_ajax_form_id_tipo_evento_validate_evento_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_id_tipo_evento_evento__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_id_tipo_evento_evento__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
        ...
        displayChange_field("evento_", "", status);
}

function displayChange_row(row, status) {
        displayChange_field_id_tipo_evento_(row, status);
        displayChange_field_evento_(row, status);
}
...
function displayChange_field_evento_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
        $("#id_form_id_tipo_evento_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_id_tipo_evento')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
