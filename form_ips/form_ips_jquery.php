...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["nombre_ips_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["nombre_ips_" + iSeqRow] && scEventControl_data["nombre_ips_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["nombre_ips_" + iSeqRow] && scEventControl_data["nombre_ips_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_ips_' + iSeqRow).bind('blur', function() { sc_form_ips_id_ips__onblur('#id_sc_field_id_ips_' + iSeqRow, iSeqRow) })
                                     .bind('change', function() { sc_form_ips_id_ips__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_ips_id_ips__onfocus(this, iSeqRow) });
  $('#id_sc_field_nombre_ips_' + iSeqRow).bind('blur', function() { sc_form_ips_nombre_ips__onblur('#id_sc_field_nombre_ips_' + iSeqRow, iSeqRow) })
                                         .bind('change', function() { sc_form_ips_nombre_ips__onchange(this, iSeqRow) })
                                         .bind('focus', function() { sc_form_ips_nombre_ips__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_ips_id_ips__onblur(oThis, iSeqRow) {
  do_ajax_form_ips_validate_id_ips_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_ips_id_ips__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_ips_id_ips__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_ips_nombre_ips__onblur(oThis, iSeqRow) {
  do_ajax_form_ips_validate_nombre_ips_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_ips_nombre_ips__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_ips_nombre_ips__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
        ...
        displayChange_field("nombre_ips_", "", status);
}

function displayChange_row(row, status) {
        displayChange_field_id_ips_(row, status);
        displayChange_field_nombre_ips_(row, status);
}
...
function displayChange_field_nombre_ips_(row, status) {
    var fieldId;
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_ips')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
