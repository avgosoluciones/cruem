<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="id_ips_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['id_ips_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_ips"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_ips']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_ips']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_ips']['dashboard_info']['parent_widget'] . "']\")";
...
function scForm_inline_submit(y, z) {
        Nm_Proc_Atualiz = true;
        do_ajax_form_ips_submit_form(y, z);
