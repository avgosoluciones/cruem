...
$old_error_handler = set_error_handler("form_medico_mob_scriptcase_error_handler");

function form_medico_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_medico_mob']['contr_erro']) && $_SESSION['scriptcase']['form_medico_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_medico_mob->contr_form_medico_mob->Apl_com_erro = true;
            if ($inicial_form_medico_mob->contr_form_medico_mob->NM_ajax_flag)
            {
                $inicial_form_medico_mob->contr_form_medico_mob->NM_ajax_info['errList']['geral_form_medico_mob'][] = $msg;
                return;
            }
            form_medico_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_medico_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_medico_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['form_medico_mob'])  ? $_SESSION['scriptcase']['error_icon']['form_medico_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_medico_mob']) && '' != $_SESSION['scriptcase']['error_close']['form_medico_mob']) ? $_SESSION['scriptcase']['error_close']['form_medico_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_medico_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['form_medico_mob']) ? $_SESSION['scriptcase']['error_span_title']['form_medico_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_medico_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_medico_mob']) ? $_SESSION['scriptcase']['error_icon_title']['form_medico_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_medico']['error_buffer'] .= ob_get_flush();
}
