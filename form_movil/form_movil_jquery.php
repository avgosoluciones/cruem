...
function scEventControl_init(iSeqRow) {
  scEventControl_data["placa_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["placa_" + iSeqRow] && scEventControl_data["placa_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["placa_" + iSeqRow] && scEventControl_data["placa_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_movil_' + iSeqRow).bind('change', function() { sc_form_movil_id_movil__onchange(this, iSeqRow) });
  $('#id_sc_field_placa_' + iSeqRow).bind('blur', function() { sc_form_movil_placa__onblur('#id_sc_field_placa_' + iSeqRow, iSeqRow) })
                                    .bind('change', function() { sc_form_movil_placa__onchange(this, iSeqRow) })
                                    .bind('focus', function() { sc_form_movil_placa__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_movil_id_movil__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_form_movil_placa__onblur(oThis, iSeqRow) {
  do_ajax_form_movil_validate_placa_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_movil_placa__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_movil_placa__onfocus(oThis, iSeqRow) {
...
function displayChange_block_0(status) {
        displayChange_field("placa_", "", status);
}

function displayChange_row(row, status) {
        displayChange_field_placa_(row, status);
}
...
function displayChange_field_placa_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
        $("#id_form_movil_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_movil')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
