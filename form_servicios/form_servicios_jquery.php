...
function scEventControl_init(iSeqRow) {
  scEventControl_data["id_servicios_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["servicios_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["id_servicios_" + iSeqRow] && scEventControl_data["id_servicios_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_servicios_" + iSeqRow] && scEventControl_data["id_servicios_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["servicios_" + iSeqRow] && scEventControl_data["servicios_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["servicios_" + iSeqRow] && scEventControl_data["servicios_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_servicios_' + iSeqRow).bind('blur', function() { sc_form_servicios_id_servicios__onblur('#id_sc_field_id_servicios_' + iSeqRow, iSeqRow) })
                                           .bind('change', function() { sc_form_servicios_id_servicios__onchange(this, iSeqRow) })
                                           .bind('focus', function() { sc_form_servicios_id_servicios__onfocus(this, iSeqRow) });
  $('#id_sc_field_servicios_' + iSeqRow).bind('blur', function() { sc_form_servicios_servicios__onblur('#id_sc_field_servicios_' + iSeqRow, iSeqRow) })
                                        .bind('change', function() { sc_form_servicios_servicios__onchange(this, iSeqRow) })
                                        .bind('focus', function() { sc_form_servicios_servicios__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_servicios_id_servicios__onblur(oThis, iSeqRow) {
  do_ajax_form_servicios_validate_id_servicios_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_servicios_id_servicios__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_servicios_id_servicios__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_servicios_servicios__onblur(oThis, iSeqRow) {
  do_ajax_form_servicios_validate_servicios_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_servicios_servicios__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_servicios_servicios__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
        displayChange_field("id_servicios_", "", status);
        displayChange_field("servicios_", "", status);
}

function displayChange_row(row, status) {
        displayChange_field_id_servicios_(row, status);
        displayChange_field_servicios_(row, status);
}

function displayChange_field(field, row, status) {
        if ("id_servicios_" == field) {
                displayChange_field_id_servicios_(row, status);
        }
        if ("servicios_" == field) {
                displayChange_field_servicios_(row, status);
...
function displayChange_field_id_servicios_(row, status) {
    var fieldId;
}

function displayChange_field_servicios_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
        $("#id_form_servicios_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_servicios')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
