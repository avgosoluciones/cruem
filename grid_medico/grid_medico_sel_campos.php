<?php
   ...
   if(!$bol_sel_campos_include)
   {
     ...
     if (!isset($_SESSION['sc_session']))
     {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_medico']['glo_nm_path_imag_temp'] . "/sc_apl_default_CRUE.txt"))
       {
?>
...
<?php
     ...
     if (!function_exists("NM_is_utf8"))
     {
      ...
      $Sel_Cmp = new grid_medico_sel_cmp($bol_sel_campos_include); 
      $Sel_Cmp->Sel_cmp_init();
      $Sel_Cmp->Sel_cmp_init_fields();
      $Sel_Cmp->Sel_cmp_process();
     
  }
class grid_medico_sel_cmp
{
function Sel_cmp_init($bol_sel_campos_include = false, $sc_init = '', $path_img = '', $path_btn = '', $path_fields = '',$embbed = true, $tbar_pos = '')
{
   ...
   foreach ($_SESSION['sc_session'][$this->sc_init]['grid_medico']['labels'] as $ind => $dados) {
...
function Sel_cmp_process()
{
   ...
   function Sel_cmp_init_fields()
   {
      ...
      $tab_ger_campos['medico'] = "on";
      $tab_ger_campos['registro_medico'] = "on";
      ...
      if (isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_medico']['php_cmp_sel']))
      {
          foreach ($_SESSION['sc_session'][$this->sc_init]['grid_medico']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
          {
              if ($NM_cada_opc == "off")
              {
                 $tab_ger_campos[$NM_cada_field] = "none";
      ...
      if ($this->restore)
      {
          if (isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel_orig']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel_orig']))
          {
              foreach ($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel_orig'] as $NM_cada_field => $NM_cada_opc)
              {
      ...
      {
          if (isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel']))
          {
              foreach ($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
              {
                  $tab_ger_campos[$NM_cada_field] = $NM_cada_opc;
              }
          }
...
function Sel_processa_out()
{
   ...
   $arr_temp = array();
   if (!isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel']))
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel'] = array();
   }
   if($this->bol_sel_campos_include)
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_medico']['export_sel_columns']['field_order'] = $_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order'];
       $_SESSION['sc_session'][$this->sc_init]['grid_medico']['export_sel_columns']['usr_cmp_sel'] = $_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel'];
   }
   $arr_order = $_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order'];
   $_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order'] = array();
   $campos_sel = explode("@?@", $this->campos_sel);
   foreach ($campos_sel as $campo_order)
   {
       if (isset($tab_ger_campos[$campo_order]))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order'][] = $campo_order;
       }
   }
   foreach ($arr_order as $campo_order)
   {
       if (!in_array($campo_order, $_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order']))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order'][] = $campo_order;
       }
   }
   ...
   $_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel'] = $arr_temp;
   if($this->bol_sel_campos_include)
   {
       return;
   }
...
?>
   $.ajax({
    type: "POST",
    url: "grid_medico_sel_campos.php",
    data: {
          ...
          if ($this->proc_ajax)
          {
           ...
           foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
           {
               if ($NM_cada_opc != "none")
               {
                   ...
                   {
/*--- mantis 24870 ---*/
/*                       $prep_combo[strtolower($NM_cada_field)] = $_SESSION['sc_session'][$this->sc_init]['grid_medico']['labels'][$NM_cada_field]; */
...
?>
   ...
   if ($this->restore && isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order_orig']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order_orig'] as $NM_cada_field)
       {
...
<?php
   ...
   if (!$this->restore && isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order'] as $NM_cada_field)
       {
           if ($tab_ger_campos[$NM_cada_field] == "on")
           {
               $str_class = "scAppDivSelectFieldsDisabled";
...
<?php
  $disp_rest = "none";
  if (isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel']) && isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel_orig']) && $_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel'] != $_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel_orig']) {
      $disp_rest = "";
  }
  else {
      $order_orig = array();
      $order_atu  = array();
      foreach ($_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order_orig'] as $ix => $cmp) {
          if (!isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel_orig'][$cmp]) || $_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel_orig'][$cmp] != "off") {
              $order_orig[] = $cmp;
          }
      }
      foreach ($_SESSION['sc_session'][$this->sc_init]['grid_medico']['field_order'] as $ix => $cmp) {
          if (!isset($_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel'][$cmp]) || $_SESSION['sc_session'][$this->sc_init]['grid_medico']['usr_cmp_sel'][$cmp] != "off") {
              $order_atu[] = $cmp;
          }
      }
...
<HEAD>
 <TITLE><?php echo $this->Nm_lang['lang_othr_grid_title'] ?> medico</TITLE>
