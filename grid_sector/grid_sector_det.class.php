...
class grid_sector_det
{
 ...
 function monta_det()
 {
   ...
   {
       $nmgp_select .= " where  Id_Sector = $parms_det[0]" ;  
   } 
   elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
   {
       $nmgp_select .= " where  Id_Sector = $parms_det[0]" ;  
...
$nm_saida->saida("</script>\r\n");
   ...
   $nm_saida->saida("</HEAD>\r\n");
    $removeMargin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sector']['link_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sector']['link_info']['remove_margin'] ? 'margin: 0;' : '';
    $removeBorder = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sector']['link_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sector']['link_info']['remove_border'] ? 'border-width: 0;' : '';
    $removeBackground = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sector']['link_info']['remove_background']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sector']['link_info']['remove_background'] ? 'background-color: transparent; background-image: none;' : '';
   ...
   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sector']['link_info']['compact_mode']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_sector']['link_info']['compact_mode']) {
   ...
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sector']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->sector)));
   ...
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_sector_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_sector\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("</TABLE>\r\n");
   if(isset($_SESSION['scriptcase']['proc_mobile']) && $_SESSION['scriptcase']['proc_mobile'])
...
//--- 
   ...
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("       NovaJanela = window.open (\"grid_sector_doc.php?nmgp_parms=\" + campo1, \"_self\", \"resizable\");\r\n");
   $nm_saida->saida("   }\r\n");
   ...
   { 
       ...
       if ($this->nmgp_botoes['det_pdf'] == "on")
       {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Dpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_sector/grid_sector_config_pdf.php?nm_opc=pdf_det&nm_target=0&nm_cor=cor&papel=1&orientacao=1&largura=1200&conf_larg=S&conf_fonte=10&language=es&conf_socor=N&sc_ver_93=s&password=n&pdf_zip=N&KeepThis=false&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
