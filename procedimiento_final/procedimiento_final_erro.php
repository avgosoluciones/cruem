...
$old_error_handler = set_error_handler("procedimiento_final_scriptcase_error_handler");

function procedimiento_final_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['procedimiento_final']['contr_erro']) && $_SESSION['scriptcase']['procedimiento_final']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_procedimiento_final->contr_procedimiento_final->Apl_com_erro = true;
            if ($inicial_procedimiento_final->contr_procedimiento_final->NM_ajax_flag)
            {
                $inicial_procedimiento_final->contr_procedimiento_final->NM_ajax_info['errList']['geral_procedimiento_final'][] = $msg;
                return;
            }
            procedimiento_final_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function procedimiento_final_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['procedimiento_final'])  && '' != $_SESSION['scriptcase']['error_icon']['procedimiento_final'])  ? $_SESSION['scriptcase']['error_icon']['procedimiento_final']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['procedimiento_final']) && '' != $_SESSION['scriptcase']['error_close']['procedimiento_final']) ? $_SESSION['scriptcase']['error_close']['procedimiento_final'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['procedimiento_final']) && '' != $_SESSION['scriptcase']['error_span_title']['procedimiento_final']) ? $_SESSION['scriptcase']['error_span_title']['procedimiento_final'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['procedimiento_final']) && '' != $_SESSION['scriptcase']['error_icon_title']['procedimiento_final']) ? $_SESSION['scriptcase']['error_icon_title']['procedimiento_final'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['procedimiento_final']['error_buffer'] .= ob_get_flush();
}
